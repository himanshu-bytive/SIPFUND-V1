...
function ReturnsCalculator(props) {
    ...
    useEffect(() => {
        ...
        let years1 = detailedPortFolio["DP-Return1Yr"] ? Number(detailedPortFolio["DP-Return1Yr"]).toFixed(2) : 'NA'
        setAnnum(years1)
    ...
    useEffect(() => {
        let detailedPortFolio = detailsInfo ? detailsInfo[0].api : {};
        let years1 = detailedPortFolio["DP-Return1Yr"] ? Number(detailedPortFolio["DP-Return1Yr"]).toFixed(2) : 'NA'
        let years3 = detailedPortFolio["DP-Return3Yr"] ? Number(detailedPortFolio["DP-Return3Yr"]).toFixed(2) : 'NA'
        let years5 = detailedPortFolio["DP-Return5Yr"] ? Number(detailedPortFolio["DP-Return5Yr"]).toFixed(2) : 'NA'
        if (year === 1) {
            setAnnum(years1)
        }
        if (year === 3) {
    ...
    return (<View style={styles.mainbox}>
        ...
        <View style={styles.amount_box}>
            ...
            <Text style={styles.price}>Rs.{amount}</Text>
        </View>
        <View style={styles.back_sec}>
            <TouchableOpacity onPress={() => setYear(1)} style={styles.back1}>
                <Text style={year === 1 ? styles.back_year : styles.back_year2}>1Y Back</Text>
                <View style={year === 1 ? styles.back_year_text : styles.back_year_text2}></View>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setYear(3)} style={styles.back1}>
                <Text style={year === 3 ? styles.back_year : styles.back_year2}>3Y Back</Text>
                <View style={year === 3 ? styles.back_year_text : styles.back_year_text2}></View>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setYear(5)} style={styles.back1}>
                <Text style={year === 5 ? styles.back_year : styles.back_year2}>5Y Back</Text>
                <View style={year === 5 ? styles.back_year_text : styles.back_year_text2}></View>
            </TouchableOpacity>
        </View>
        <Text style={styles.rs}> Rs. {values}</Text>
