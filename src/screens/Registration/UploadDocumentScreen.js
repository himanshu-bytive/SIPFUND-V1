...
import { StyleSheet, ScrollView, View, TouchableOpacity, ActivityIndicator, BackHandler, Text } from "react-native";
        ...
        <View style={styles.container}>
            ...
            <ScrollView>
                <View style={{ alignItems: "center" }}>
                    <Text style={styles.heading}>Please Upload your documents for easy and quick Activation of your transactional account.</Text>
                </View>
                    ...
                    <ScrollView horizontal={true}>
                        {docs?.responseString.documents
                            ? docs.responseString.documents.map((item, key) => (
                                  <View key={key} style={{ marginHorizontal: 10, alignItems: "center", width: Styles.width - 50 }}>
                                      <Text style={{ marginBottom: 5 }}>{documentsMap[item.docType] ? documentsMap[item.docType] : item.docType}</Text>
                                      <Image source={{ uri: `${docs?.baseUrl}${item.fileName}` }} style={{ width: Styles.width - 50, height: 300 }} />
                                  </View>
                              ))
                            : null}
                    </ScrollView>
